...
TEXTOS: dict[str, dict[str, str]] = {
    ...
    "novidade.23": {
        "en": "the crash is found, and it was ours. The kernel log of a real "
              ...
              "the \"freeze\". The 8.4 MB comes from the commands that read "
              "the whole queue — the batch send and the card's spreadsheet — "
              "and not from the announcement, which allocates almost nothing; "
              "the announcement only looked guilty because a send follows it "
              "closely. The queue now grows as it fills: ten tracks cost "
        ...
        "pt": "o travamento foi achado, e era nosso. O log do kernel de um "
              ...
              "Os 8,4 MB vem dos comandos que leem a fila inteira — o envio "
              "do lote e a planilha do cartao —, e nao do anuncio, que quase "
              "nao aloca nada; o anuncio so parecia culpado porque um envio "
              "vem logo atras dele. Agora a fila cresce "
